  ...
  <button @click.stop="openLinkWith(`https://marketplace.rcax.io/asset/${listing.token.contract_address}:${listing.token.id}`)" class="flex items-center group">
    <template v-if="listing.payment_token.symbol === 'ETH'">
      ...
      <div class="flex gap-1">
        <span class="text-header font-medium">{{ (listing.payment_token.base_price / ETH_TO_GWEI_MODIFIER).toFixed(4).replace(/\.?0+$/, '') }}</span>
        <span class="text-amber-500 font-medium">{{ ethereumInLocalCurrency(listing.payment_token.base_price) }}</span>
        <span class="text-details font-medium">#{{ listing.token.mint_number }}</span>
      </div>
    </template>
    <template v-else-if="listing.payment_token.symbol === 'MATIC'">
      ...
      <div class="flex gap-1">
        <span class="text-header font-medium">{{ (listing.payment_token.base_price / ETH_TO_GWEI_MODIFIER).toFixed(4).replace(/\.?0+$/, '') }}</span>
        <span class="text-amber-500 font-medium">{{ ethereumInLocalCurrency(listing.payment_token.base_price / ethereumPriceMap.get("MATIC")) }}</span>
        <span class="text-details font-medium">#{{ listing.token.mint_number }}</span>
      </div>
    </template>
  </button>
