<template>
  <button @click.stop="openLinkWith(`https://opensea.io/assets/matic/${listing.token.contract_address}/${listing.token.id}`)" class="flex items-center group">
    <template v-if="normalizeTokenSymbol(listing.payment_token.symbol) === 'ETH'">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="currentColor" class="w-3 h-3 text-purple-500"><path d="M311.9 260.8L160 353.6 8 260.8 160 0l151.9 260.8zM160 383.4L8 290.6 160 512l152-221.4-152 92.8z"></path></svg>
      <div class="flex gap-1">
        <span class="text-white font-medium">{{ (listing.payment_token.base_price / ETH_TO_GWEI_MODIFIER).toFixed(4).replace(/\.?0+$/, '') }}</span>
        <span class="text-white/40">{{ ethereumInLocalCurrency(listing.payment_token.base_price) }}</span>
        <span class="text-white/20">#{{ listing.token.mint_number }}</span>
      </div>
    </template>
    <template v-else-if="normalizeTokenSymbol(listing.payment_token.symbol) === 'MATIC'">
      ...
      <div class="flex gap-1">
        <span class="text-white font-medium">{{ (listing.payment_token.base_price / ETH_TO_GWEI_MODIFIER).toFixed(4).replace(/\.?0+$/, '') }}</span>
        <span class="text-white/40">{{ ethereumInLocalCurrency(listing.payment_token.base_price / ethereumPriceMap.get("MATIC")) }}</span>
        <span class="text-white/20">#{{ listing.token.mint_number }}</span>
      </div>
    </template>
  </button>
