<template>
  <div v-if="showingBarMarketInfo" class="sm:py-2 border-b border-tertiary w-full text-xs overflow-hidden" ref="barMarketInfo" :class="{ 'page-mobile-padding-top': Capacitor.isNativePlatform() }">
    <div class="px-4 sm:px-6 py-1 flex whitespace-nowrap items-center overflow-x-auto scrollbar-hide">
      <div class="inline-flex gap-2 sm:gap-3">
        ...
        <div class="flex items-center gap-1">
          ...
          <span class="text-amber-500 font-medium">{{ gweiInLocalCurrency(cone) }}</span>
        </div>
        <div class="flex items-center gap-1">
      ...
      <template v-if="!Capacitor.isNativePlatform()">
        <a href="https://whitepaper.rcax.io/" target="_blank" class="hidden md:flex gap-1 ml-auto mx-1 px-3 py-1.5 text-white/80 hover:text-header font-semibold rounded-lg duration-200 cursor-pointer">
