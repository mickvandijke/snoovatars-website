  ...
  <VirtualGrid :items="items">
    <template #default="{ item, index }">
      <AvatarCard :item="{ name: item.token.name, contract_address: item.token.contract_address, image: item.token.image }" :series-stats="getSeriesStats(item.token.name)">
        ...
        <div class="flex items-center gap-1 font-bold text-[0.8rem] md:text-[1.1rem] overflow-hidden">
          ...
          <div class="text-amber-500" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $timeAgo(new Date(item.date_listed)) }} ago</div>
        </div>
      </AvatarCard>
    </template>
  </VirtualGrid>
