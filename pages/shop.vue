  ...
  <div class="mt-3 md:mt-0 flex flex-col gap-6 w-full">
    ...
    <div class="px-2 md:px-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8 gap-2">
      <template v-for="item in sortedItems">
        <div class="p-4 w-full flex flex-col gap-2 border border-secondary rounded-2xl overflow-hidden">
          ...
          <div>
            <div class="grid grid-cols-2 divide-x divide-white/10 py-1.5 bg-secondary/40 rounded-lg">
          ...
          <div class="w-full">
            <template v-if="item['edge']['node']['status'] === 'PENDING'">
              <a :href="`https://www.reddit.com/avatar/shop/product/storefront_nft_${item['edge']['node']['item']['benefits']['avatarOutfit']['id']}`" target="_blank" class="flex justify-center items-center gap-1 px-2 py-2 w-full bg-secondary hover:bg-white/10 text-white/40 text-xs font-semibold uppercase rounded-lg cursor-pointer duration-300">
