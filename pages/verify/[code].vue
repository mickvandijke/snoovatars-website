    ...
    <template v-if="loading">
      <div class="min-h-full w-full flex flex-col items-center">
        <button disabled type="button" class="text-white border border-amber-600 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 inline-flex items-center">
          ...
          <span class="text-neutral-300">Awaiting verification...</span>
        </button>
      </div>
    </template>
    <template v-else>
      <div class="min-h-full w-full flex flex-col items-center">
        <div class="p-3 bg-neutral-800 text-neutral-400 rounded-2xl">{{ message }}</div>
      </div>
    </template>
  </div>
</template>
...
import {verifyUser} from "~/composables/api/user";
...
function verify() {
  verifyUser(route.params.code as string)
      .then(() => {
        message.value = "Verification success! You will be redirected to the login page."
        loading.value = false;
